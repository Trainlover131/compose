...
from apps.api.services.render_compiler import (
    ...
    _default_motion_for_broll,
    _default_motion_for_overlay,
    _motion_seed,
    assert_no_forbidden_filters,
...
# ====================================================================
# Task C — Motion effects system
# ====================================================================


class TestMotionSeed:
    """Deterministic seeding for motion selection."""

    def test_same_inputs_same_seed(self):
        s1 = _motion_seed("/tmp/a.mp4", 1.0, 3.0)
        s2 = _motion_seed("/tmp/a.mp4", 1.0, 3.0)
        assert s1 == s2

    def test_different_inputs_different_seed(self):
        s1 = _motion_seed("/tmp/a.mp4", 1.0, 3.0)
        s2 = _motion_seed("/tmp/b.mp4", 1.0, 3.0)
        assert s1 != s2

    def test_seed_is_int(self):
        s = _motion_seed("clip.mp4", 0.0, 5.0)
        assert isinstance(s, int)


class TestDefaultMotionSelection:
    """Default motion is chosen deterministically."""

    def test_broll_default_type_in_allowed(self):
        bc = {"path": "/tmp/b.mp4", "start": 2.0, "end": 5.0}
        md = _default_motion_for_broll(bc)
        assert md["type"] in ("fade", "micro_push", "slide")

    def test_overlay_default_type_in_allowed(self):
        oi = {"path": "/tmp/ov.png", "start": 1.0, "end": 3.0}
        md = _default_motion_for_overlay(oi)
        assert md["type"] in ("fade", "slide", "pop")

    def test_broll_default_deterministic(self):
        bc = {"path": "/tmp/det.mp4", "start": 3.0, "end": 6.0}
        m1 = _default_motion_for_broll(bc)
        m2 = _default_motion_for_broll(bc)
        assert m1 == m2

    def test_overlay_default_deterministic(self):
        oi = {"path": "/tmp/det.png", "start": 1.0, "end": 2.0}
        m1 = _default_motion_for_overlay(oi)
        m2 = _default_motion_for_overlay(oi)
        assert m1 == m2

    def test_broll_default_strength_is_subtle(self):
        bc = {"path": "/tmp/s.mp4", "start": 0.0, "end": 3.0}
        md = _default_motion_for_broll(bc)
        assert md["strength"] == "subtle"


class TestForbiddenFilterGuard:
    """Safety: motion fragments must not contain forbidden tokens."""

    def test_clean_fragment_passes(self):
        # Should not raise
        assert_no_forbidden_filters("crop=w=100:h=100:x=0:y=0:eval=frame,scale=1080:1920")

    def test_forbidden_fps_raises(self):
        with pytest.raises(ValueError, match="forbidden token"):
            assert_no_forbidden_filters("fps=30,scale=1080:1920")

    def test_forbidden_setpts_raises(self):
        with pytest.raises(ValueError, match="forbidden token"):
            assert_no_forbidden_filters("setpts=PTS/2")

    def test_forbidden_atempo_raises(self):
        with pytest.raises(ValueError, match="forbidden token"):
            assert_no_forbidden_filters("atempo=1.5")


class TestCompileMotionForBroll:
    """B-roll motion compilation."""

    def test_micro_push_produces_crop_scale(self):
        bc = {"path": "/tmp/b.mp4", "start": 2.0, "end": 5.0,
              "motion": {"type": "micro_push", "in_dur": 0.15, "out_dur": 0.15,
                         "strength": "subtle", "seed": 42}}
        result = compile_motion_for_broll(bc, 2.0, 5.0)
        assert "crop=" in result
        assert "scale=1080:1920" in result
        assert "eval=frame" in result

    def test_none_type_returns_empty(self):
        bc = {"path": "/tmp/b.mp4", "start": 1.0, "end": 3.0,
              "motion": {"type": "none"}}
        result = compile_motion_for_broll(bc, 1.0, 3.0)
        assert result == ""

    def test_fade_type_returns_empty_for_broll(self):
        """B-roll fade is a no-op (fullscreen, no alpha fade)."""
        bc = {"path": "/tmp/b.mp4", "start": 1.0, "end": 3.0,
              "motion": {"type": "fade", "in_dur": 0.15, "out_dur": 0.15}}
        result = compile_motion_for_broll(bc, 1.0, 3.0)
        assert result == ""

    def test_slide_produces_crop_scale(self):
        bc = {"path": "/tmp/b.mp4", "start": 1.0, "end": 4.0,
              "motion": {"type": "slide", "in_dur": 0.15, "out_dur": 0.15,
                         "dir": "right", "strength": "subtle", "seed": 7}}
        result = compile_motion_for_broll(bc, 1.0, 4.0)
        assert "crop=" in result
        assert "scale=1080:1920" in result

    def test_no_forbidden_tokens_in_micro_push(self):
        bc = {"path": "/tmp/b.mp4", "start": 0.0, "end": 3.0,
              "motion": {"type": "micro_push", "in_dur": 0.15, "out_dur": 0.15,
                         "strength": "subtle", "seed": 99}}
        result = compile_motion_for_broll(bc, 0.0, 3.0)
        for tok in _FORBIDDEN_MOTION_TOKENS:
            assert tok not in result, f"Forbidden token {tok} in broll motion"

    def test_fallback_on_error(self):
        """If motion dict is malformed, should return empty (no crash)."""
        bc = {"path": "/tmp/b.mp4", "start": 1.0, "end": 3.0,
              "motion": {"type": "micro_push"}}  # missing fields but should still work
        result = compile_motion_for_broll(bc, 1.0, 3.0)
        # Should succeed or return empty, never crash
        assert isinstance(result, str)


class TestCompileMotionForOverlay:
    """Overlay motion compilation."""

    def test_fade_adds_alpha_fade(self):
        oi = {"path": "/tmp/ov.png", "start": 1.0, "end": 3.0,
              "motion": {"type": "fade", "in_dur": 0.12, "out_dur": 0.12}}
        prep, x, y = compile_motion_for_overlay(oi, 1.0, 3.0, 100, 200, 150, 200)
        assert "fade=t=in" in prep
        assert "fade=t=out" in prep
        assert "alpha=1" in prep
        assert x == "100"
        assert y == "200"

    def test_slide_animates_x_or_y(self):
        oi = {"path": "/tmp/ov.png", "start": 2.0, "end": 4.0,
              "motion": {"type": "slide", "in_dur": 0.12, "out_dur": 0.12,
                         "dir": "left"}}
        prep, x, y = compile_motion_for_overlay(oi, 2.0, 4.0, 100, 200, 150, 200)
        # x should be an expression (not just "100")
        assert "100" in x  # contains base position
        assert len(x) > 5  # it's an expression, not just the number
        assert y == "200"  # y unchanged for horizontal slide

    def test_pop_adds_scale(self):
        oi = {"path": "/tmp/ov.png", "start": 1.0, "end": 2.5,
              "motion": {"type": "pop", "in_dur": 0.12, "out_dur": 0.12}}
        prep, x, y = compile_motion_for_overlay(oi, 1.0, 2.5, 50, 100, 200, 250)
        assert "scale=" in prep
        assert "eval=frame" in prep
        assert "0.94" in prep  # scale start

    def test_subtle_wobble_animates_xy(self):
        oi = {"path": "/tmp/ov.png", "start": 3.0, "end": 6.0,
              "motion": {"type": "subtle_wobble", "in_dur": 0.12, "out_dur": 0.12}}
        prep, x, y = compile_motion_for_overlay(oi, 3.0, 6.0, 200, 300, 100, 100)
        assert "sin(" in x
        assert "sin(" in y
        assert "fade=t=in" in prep  # also has fade

    def test_none_returns_static(self):
        oi = {"path": "/tmp/ov.png", "start": 1.0, "end": 2.0,
              "motion": {"type": "none"}}
        prep, x, y = compile_motion_for_overlay(oi, 1.0, 2.0, 50, 80, 100, 100)
        assert prep == ""
        assert x == "50"
        assert y == "80"

    def test_no_forbidden_tokens_in_overlay_fade(self):
        oi = {"path": "/tmp/ov.png", "start": 0.0, "end": 2.0,
              "motion": {"type": "fade", "in_dur": 0.15, "out_dur": 0.15}}
        prep, _, _ = compile_motion_for_overlay(oi, 0.0, 2.0, 0, 0, 100, 100)
        for tok in _FORBIDDEN_MOTION_TOKENS:
            assert tok not in prep, f"Forbidden token {tok} in overlay motion"

    def test_no_forbidden_tokens_in_overlay_slide(self):
        oi = {"path": "/tmp/ov.png", "start": 1.0, "end": 3.0,
              "motion": {"type": "slide", "in_dur": 0.12, "out_dur": 0.12,
                         "dir": "right"}}
        prep, x, y = compile_motion_for_overlay(oi, 1.0, 3.0, 50, 50, 200, 200)
        combined = f"{prep},{x},{y}"
        for tok in _FORBIDDEN_MOTION_TOKENS:
            assert tok not in combined, f"Forbidden token {tok} in overlay slide"

    def test_fallback_on_error_returns_fade(self):
        """Malformed motion should fall back to fade, not crash."""
        oi = {"path": "/tmp/ov.png", "start": 1.0, "end": 2.0,
              "motion": {"type": "pop"}}  # missing in_dur/out_dur but has defaults
        prep, x, y = compile_motion_for_overlay(oi, 1.0, 2.0, 50, 80, 100, 100)
...
class TestBrollMotionInFiltergraph:
    """Motion tail is correctly wired into b-roll filtergraph."""

    def test_non_heavy_clips_get_motion_tail(self):
        """Non-HEAVY clips should have motion filters after look filter."""
        clips = [{"path": "/tmp/motion_test.mp4", "start": 2.0, "end": 5.0,
                  "motion": {"type": "micro_push", "in_dur": 0.15, "out_dur": 0.15,
                             "strength": "subtle", "seed": 42}}]
        filters, last, _ = build_broll_filtergraph_entries(clips, 1, "[0:v]")
        joined = ";".join(filters)
        # The look line should contain crop (from micro_push)
        look_lines = [f for f in filters if f.endswith("[b0_look]")]
        assert len(look_lines) == 1
        assert "crop=" in look_lines[0]
        assert "scale=1080:1920" in look_lines[0]

    def test_motion_none_no_extra_filters(self):
        """motion=none should produce the same as original (look only)."""
        clips = [{"path": "/tmp/no_motion.mp4", "start": 1.0, "end": 3.0,
                  "motion": {"type": "none"}}]
        filters, _, _ = build_broll_filtergraph_entries(clips, 1, "[0:v]")
        look_lines = [f for f in filters if f.endswith("[b0_look]")]
        assert len(look_lines) == 1
        # Should NOT have crop= from motion (only look preset)
        assert "crop=" not in look_lines[0] or "crop=1080:1920" in look_lines[0]

    def test_labels_unchanged_with_motion(self):
        """Labels [b{i}_raw], [b{i}_look], [b{i}_out] must still exist."""
        clips = [{"path": "/tmp/lbl.mp4", "start": 3.0, "end": 6.0}]
        filters, last, _ = build_broll_filtergraph_entries(clips, 1, "[0:v]")
        joined = ";".join(filters)
        assert "[b0_raw]" in joined
        assert "[b0_look]" in joined
    ...
    def test_enable_window_unchanged_with_motion(self):
        """The overlay enable='between(t,...)' must be exactly same times."""
        clips = [{"path": "/tmp/en.mp4", "start": 4.5, "end": 7.2}]
        filters, _, _ = build_broll_filtergraph_entries(clips, 1, "[0:v]")
        overlay_lines = [f for f in filters if "overlay=" in f]
        assert len(overlay_lines) == 1
        assert "enable='between(t,4.500,7.200)'" in overlay_lines[0]

    def test_heavy_clips_unaffected_by_motion(self):
        """HEAVY clips should still use zoompan, not the new motion system."""
        clips = [{"path": "/tmp/heavy.mp4", "start": 2.0, "end": 5.0,
                  "motion": {"type": "micro_push"}}]
        with patch(
            "apps.api.services.render_compiler.choose_broll_look", return_value="HEAVY"
        ):
            filters, _, _ = build_broll_filtergraph_entries(clips, 1, "[0:v]")
        ...
        # HEAVY path should NOT use our crop-based motion
        look_lines = [f for f in filters if f.endswith("[b0_look]")]
        assert len(look_lines) == 1
        assert "zoompan=" in look_lines[0]
