...
PLANNER_SYSTEM_PROMPT = (
    ...
    "   IMPORTANT: Always set captions.style_id to the preset's style_id "
    "(usually \"helvetica_punch\"). Only use a different style_id if the "
    "user explicitly asks for cinematic, podcast, luxury, or study style.\n"
    ...
    "   EMPHASIS RULE — If the user asks for a specific font \"on emphasis\" "
    "or \"for emphasis\", set captions.style.font_emphasis to that font AND "
    "set captions.style.pause_emphasis.enabled=true. The base/primary font "
    "stays as the preset default (do NOT set font_primary unless the user "
    "explicitly asks to change the base/primary font).\n"
    "FONT INTENT MAP — choose fonts by vibe (prefer first available). "
    "If user requests a specific font name, try it first; if unavailable, "
    "fall back to the closest intent chain.\n"
    "  * sans / helvetica / clean / modern → "
    "Google Sans, Inter, Roboto Condensed, Liberation Sans, DejaVu Sans, Noto Sans\n"
    "  * serif / classic / editorial / fancy / playfair → "
    "Playfair Display, EB Garamond, DejaVu Serif, Noto Serif\n"
    "  * handwritten / casual → "
    "Shadows Into Light, Comic Neue, DejaVu Sans, Noto Sans\n"
    "  * script / cursive / elegant signature → "
    "Pinyon Script, Comic Neue Italic, DejaVu Sans\n"
    "  * mono / code / terminal → "
    "JetBrains Mono, DejaVu Sans Mono, Noto Mono\n"
    "  * gothic / blackletter / medieval → "
    "UnifrakturMaguntia, DejaVu Serif\n"
    "  * condensed / narrow → "
    "Roboto Condensed, Google Sans, Inter\n"
    ...
    "- Available template_ids: kpi-counter, kpi-chart, line-chart, quote-highlight, "
    ...
    "- Money/Revenue/ARR/Cost/Price numbers WITH a rising/falling line → kpi-chart\n"
    "- Money/Revenue/ARR/Cost/Price numbers (standalone counter) → kpi-counter\n"
    "- Percentages/Growth trends over time → line-chart or kpi-chart\n"
    ...
    "- kpi-chart: {label, value, prefix?, suffix?, points:[{x,label?,y}], accentColor?, bg}\n"
    "  USE kpi-chart when user wants BOTH a big number AND a rising/falling line.\n"
    "  The chart shows a line rising to the final value with a ghost number behind.\n"
    "  points should have 3-5 entries showing progression toward the final value.\n"
